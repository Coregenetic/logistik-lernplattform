...
async function showInhalte() {
  ...
  const [
    ...
    db.from('unterrichtsstunden').select('id, kw, thema, lernfeld_id').order('datum'),
  ]);

  window._lf      = lernfelder;
  window._kapitel = kapitel;
  window._faecher = faecher;
  window._stunden = stunden;
...
function showAddForm() {
  ...
  const std = window._stunden || [];
  const html = `
    <div class="card" style="margin-bottom:20px; border: 2px solid var(--accent)">
      <h3>📄 Neuen Inhalt hinzufügen</h3>
      <div class="form-group">
        <label class="form-label">Ziel</label>
        <select class="form-input" id="new-ziel" onchange="toggleStundenSelect(this.value)">
      ...
      <div class="form-group" id="stunden-select-wrap" style="display:none">
        <label class="form-label">Unterrichtsstunde (optional)</label>
        <select class="form-input" id="new-stunde">
          <option value="">— Keine Stunde zuweisen —</option>
          ${std.map(s=>`<option value="${s.id}">KW ${s.kw}: ${s.thema}</option>`).join('')}
        </select>
      </div>
      <div class="form-group">
...
async function saveInhalt() {
  ...
  const stundeEl = document.getElementById('new-stunde');
  const stundeId = stundeEl?.value ? parseInt(stundeEl.value) : null;
  if (!titel || !text) return;
  if (ziel.startsWith('lf-')) {
    const obj = { lernfeld_id: ziel.replace('lf-',''), typ, titel, inhalt:{text}, erstellt_von:USER.id };
    if (stundeId) obj.stunde_id = stundeId;
    await db.from('inhalte').insert(obj);
  } else {
...
// Stunden-Select nur bei Lernfeld-Auswahl anzeigen
function toggleStundenSelect(ziel) {
  const wrap = document.getElementById('stunden-select-wrap');
  if (!wrap) return;
  wrap.style.display = ziel.startsWith('lf-') ? 'block' : 'none';
}
